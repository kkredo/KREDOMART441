...
$(document).ready(function() { //not sure what this actually means
    ...
    $("button").click(function() { //function for onclick event
        ...
        var num = Math.floor(1 + Math.random() * 4) // get a random number each time the button is clicked
            // use the num from above to get the random image
        $(allHorses[num].theSelector).attr("src", allHorses[num].theImagePath); //calling allHorses and establishing 0 for horse image, using attribute because src didnt work
        //after attr calling the path of what image we want
        setInterval(moveSquare, 1000); //sets how many times the square moves
        $(allHorses[num].theSelector).fadeOut().fadeIn(); //used chaining here to join the fade in and out
    });
});
